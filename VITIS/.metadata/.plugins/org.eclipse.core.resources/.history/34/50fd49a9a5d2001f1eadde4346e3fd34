...
int main(){
	...
	int timeout = 1000000; // Adjust as necessary
	while (XAxiDma_Busy(&dma_text, XAXIDMA_DMA_TO_DEVICE) && --timeout);

	if (timeout == 0) {
	    xil_printf("Timeout: DMA TEXT MM2S transfer not completed\r\n");
	}

	timeout = 1000000;
	while (XAxiDma_Busy(&dma_key, XAXIDMA_DMA_TO_DEVICE) && --timeout);
	if (timeout == 0) {
	    xil_printf("Timeout: DMA KEY MM2S transfer not completed\r\n");
	}

	timeout = 1000000;
	while (XAxiDma_Busy(&dma_output, XAXIDMA_DEVICE_TO_DMA) && --timeout);
	if (timeout == 0) {
	    xil_printf("Timeout: DMA OUTPUT S2MM transfer not completed\r\n");
	}
